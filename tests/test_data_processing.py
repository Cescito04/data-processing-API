...
class TestDataProcessing(unittest.TestCase):
    def setUp(self):
        ...
        self.data_with_duplicates = pd.DataFrame(
            {"id": [1, 2, 2, 3], "value": ["a", "b", "b", "c"]}
        )

        # Data with outliers
        self.data_with_outliers = pd.DataFrame({"values": [1, 2, 3, 100, 2, 3, 1, 200]})
    ...
    def test_handle_missing_values(self):
        ...
        self.assertEqual(
            df_median["numeric1"].iloc[3], self.test_data["numeric1"].median()
        )
    ...
    def test_remove_duplicates(self):
        ...
        # Test with subset
        df_no_dupes_subset = remove_duplicates(
            self.data_with_duplicates, subset=["value"]
        )
